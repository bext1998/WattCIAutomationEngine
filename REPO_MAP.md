...
- Issue #2～#9、#24（Phase 1 Must Have）與 #29、#30、#31、#32（對抗式審查回溯，含總覽）、#10、#13、#41、#44、#46、#47、#48、#49、#50 皆已完成並關閉／合併。目前開啟中：[Issue #37](https://github.com/bext1998/WattCIAutomationEngine/issues/37)（P2／security，無阻塞，待排程）；新開候選 [#54](https://github.com/bext1998/WattCIAutomationEngine/issues/54)（CLI 安裝精靈）／[#56](https://github.com/bext1998/WattCIAutomationEngine/issues/56)（Agent Skill 打包，P2）／[#57](https://github.com/bext1998/WattCIAutomationEngine/issues/57)（文件站分兩層讀者）。GitHub repo 現已建立 P0～P4 優先級標籤（`MAZE_PROJECT.md` 先前記錄的「無標籤」已過期）。
- [Issue #47](https://github.com/bext1998/WattCIAutomationEngine/issues/47)／[#48](https://github.com/bext1998/WattCIAutomationEngine/issues/48)／[#49](https://github.com/bext1998/WattCIAutomationEngine/issues/49)／[PR #53](https://github.com/bext1998/WattCIAutomationEngine/pull/53)／[PR #55](https://github.com/bext1998/WattCIAutomationEngine/pull/55)：官方網站與文件站上線，`https://bext1998.github.io/WattCIAutomationEngine/`。新增 `site/`（首頁＋四篇使用者導向文件：快速開始、Pipeline 撰寫、CLI 參考、排錯指引）、`site/CONTENT_ARCHITECTURE.md`（#47 內容架構決策：`docs/spec.md` 為唯一技術契約權威，網站文件為非契約使用指南）、`.github/workflows/deploy-pages.yml`（push 到 main 且改動 `site/**` 時自動部署，已驗證成功）。純 HTML/CSS/原生 JS，零依賴。文案經 whisk-badger 兩輪審查、16 處「講人話」修正（PR #55）。
- [Issue #50](https://github.com/bext1998/WattCIAutomationEngine/issues/50)／[PR #51](https://github.com/bext1998/WattCIAutomationEngine/pull/51)：裸執行 `watt`（無旗標、無子指令、非 `--help`／`-h`）新增 `cmd/watt/banner.go`，印出 ASCII 品牌橫幅／雙語標語／新手上路提示；終端機支援 Unicode 時用 ANSI Shadow 字型與 `windows.WriteConsole`（處理 short write），偵測不到時整組（橫幅＋標語＋提示）降級為純 ASCII／純英文版本，判定依據 `GetConsoleMode`＋`GetCurrentConsoleFontEx`／`TMPF_TRUETYPE`。`--version`／`--help`／`-h`／`run`／`check`／`--output json` 不受影響。spec.md 同步新增 F-25（v1.5 → v1.6）。已知未驗證：ASCII 降級分支只靠單元測試注入假資料驗證，未在真正舊版點陣字型 console 環境跑過真實案例（Windows 無可靠字型能力偵測 API，設計階段已承認此限制）。
...
- 對抗式審查回溯已全數完成：[Issue #29](https://github.com/bext1998/WattCIAutomationEngine/issues/29) 總覽、#30、#31、#32 皆已關閉；[Issue #37](https://github.com/bext1998/WattCIAutomationEngine/issues/37)（P2／security，由 #31 衍生）獨立待排程，無阻塞。
